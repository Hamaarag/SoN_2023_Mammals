plot_model_effect <- function (P.anal, m, eff2plot, plot_points=TRUE, plot_residuals=FALSE, export_plot=TRUE, ylabel = NULL, fontname = "Almoni ML v5 AAA",
                               fontsize=22, pdf_width=160, outpath = "../output/", at_list = NULL) {
  ...
  # predlabels <- hebrew_levels }
   if (eff2plot=="rescaled_Time.Diff") {
    predlabels <- as.character(as.numeric(c("2012","2014", "2016", "2018", "2020", "2022")))
  ...
  eval(parse(text=eval(expr = paste0("effplot <- effect_plot(model = m, data=P.anal, pred = ",
                                     eff2plot,
                                     ", interval = T,plot.points = plot_points, jitter = c(0.1,0), point.size = 3, cat.pred.point.size = 4, partial.residuals = plot_residuals, colors = 'Qual1', main.title = main_title, line.colors = 'black', point.alpha = 0.25, x.label = xlabel, y.label = ylabel, at = at_list)"))))
  if (eff2plot=="rescaled_Time.Diff") {
    #All other units
    effplot <- effplot + scale_x_continuous(breaks=c(-1.86423,-1.233466888,-0.601840099,0.028922631,0.66054942,1.291312151), labels = predlabels)
